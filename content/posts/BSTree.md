...
**其中LR，RL都可以利用两次LL或者RR公式直接代替，看代码**
...
![image-20260201134035521](https://bucket-qjy.oss-cn-qingdao.aliyuncs.com/picture/202602011340604.png)

这里找到了目标Node，为什么还要分左右根，如果要删直接传node不好吗

解答：

本质上还是和我们之前聊的**指针引用的底层原理**直接相关。

​	1. 直接传 `node` 为什么不行？

`deleteByMerging` 的参数是 `BSTNode<T>*&`（指针的引用），它的核心要求是：**必须传入 “原指针变量本身”**，而不是 “指向目标节点的副本指针”。

- `node` 是函数里的一个普通指针变量，它只是**指向目标节点的副本**，并不是父节点的 `left`/`right` 指针，也不是根节点 `root` 指针本身。
- 如果直接把 `node` 传给 `deleteByMerging`，因为是值传递，函数里对 `node` 的修改只会作用于这个副本，**不会改变父节点的指针指向**，导致树结构出现 “断链”。

举个直观的例子：

假设父节点 `prev` 的 `left` 指针指向目标节点 `node`。

- 如果直接传 `node`，`deleteByMerging` 里执行 `node = node->left` 只是修改了函数内的副本指针，`prev->left` 仍然指向原来的目标节点，删除后父节点的指针没有更新，树结构就错了。
- 但如果传 `prev->left`（指针的引用），`deleteByMerging` 里的修改会直接改变 `prev->left` 的值，让它指向新的子节点，树结构才能正确衔接。

------

#### 2. 为什么要分「根、左、右」三种情况？

这是为了找到**父节点指向目标节点的那个 “原指针变量”**，然后把它的引用传给 `deleteByMerging`：

- **目标节点是根节点**：需要修改的是树的根指针 `root`，所以直接传 `root`。
- **目标节点是父节点的左孩子**：需要修改的是父节点的 `left` 指针，所以传 `prev->left`。
- **目标节点是父节点的右孩子**：需要修改的是父节点的 `right` 指针，所以传 `prev->right`。

只有这样，`deleteByMerging` 才能通过指针引用，直接修改这些原指针变量的值，确保删除节点后，树的父节点能正确指向新的子节点，结构不会断裂。

------

#### 3. 核心结论

`deleteByMerging` 要的不是 “指向目标节点的指针”，而是 “父节点指向目标节点的那个指针变量的引用”。分「根、左、右」就是为了精准定位这个需要修改的指针变量，而直接传 `node` 做不到这一点。
