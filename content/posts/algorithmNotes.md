...
[P1381 单词背诵 - 洛谷](https://www.luogu.com.cn/problem/P1381)

核心问题：就是要在一个序列B上找到一个区间，这个区间包含序列A尽可能多的元素数目，在这个情况下尽量减少这歌合法区间的长度

思路：

就是要两个指针动态维护一个区间的长度，当右指针指向的是目标序列中的元素的时候，把他包含进去，然后看能否优化这个序列长度，这样让这个区间在这个两个指针维护下动态的沿着序列B前进，**可以考虑双指针实现动态窗口**
...
首先滑动窗口的题型主要分为两大类：1. 求最优区间   2.求合法区间个数

首先如何判断一个题目是否可以使用双指针来求解：

**单调性：当右指针 `right` 向右移动时，左指针 `left` 只会向右移动，绝对不会向左回退。**
...
1. **合法窗口尽可能包含序列A元素**，题目4
2. **合法窗口必须包含序列A所有元素**，题目5

**更具变式不同，他们的上述三个核心关键点也就不同**
...
题目4要求的合法窗口是尽可能包含A序列的元素

**第一优先级**：包含**最多**的「不同目标单词」（重复只算 1 个，`sum` 是不同单词的数量，要最大）

**第二优先级**：在 `sum` 最大的前提下，子串**最短**

✅ 关键：**不是必须包含所有 n 个目标单词**，是「尽可能多」！比如 n=5，最多找到 3 个，就以 3 为目标找最短。

题目5要求的合法窗口是必须包含A序列的元素

**前置条件**：必须**完全包含**t 中所有字符（包括重复次数，比如 t="aa"，子串至少要有 2 个 a）

**第二优先级**：在满足完全包含的前提下，子串**最短**	

✅ 关键：**不满足完全包含的子串直接作废**，只在合法窗口里找最短。





第一题：LeetCode 76 最小覆盖子串（强制包含所有字符）

1. 什么时候移动 right 扩大窗口？加入字符更新什么？

- **移动时机**：**无任何条件，一直向右移动**，直到遍历完字符串 `s`

- **更新数据**：

  1. 若当前字符是目标字符 → `cnt[字符]++`
  2. 若字符数量刚好达标 → `valid++`（记录达标字符种类）

  

2. 什么时候暂停扩大，移动 left 缩小窗口？移出字符更新什么？

- **缩小时机**：**只有窗口完全合法时（`valid == need.size()`）**，才开始缩 left

- **更新数据**：

  1. 若当前字符是目标字符 → 先判断是否刚好达标，达标则 `valid--`
  2. `cnt[字符]--`
  3. 左指针 `i++`
...
- **仅在【合法窗口收缩的过程中】更新**
- 只有此时窗口才满足「包含所有目标字符」，不满足条件绝不更新答案
...
- **移动时机**：**无任何条件，一直向右移动**，直到遍历完文章

- **更新数据**：

  1. 若当前是目标单词 → `cnt[单词编号]++`
  2. 若单词第一次出现 → `sum++`（不同单词数 + 1）

  

2. 什么时候暂停扩大，移动 left 缩小窗口？移出字符更新什么？

- **缩小时机**：**每次 right 移动完，无条件立刻缩 left**（能缩就缩）

- **终止条件**：遇到「只出现 1 次的目标单词」就停止（删了会丢 sum）
...
1. **`sum` 增加时**：重置长度 `len = j-i+1`（旧长度作废）
2. **窗口收缩完成后**：取最小值 `len = min(len, j-i+1)`

------

终极对比总结（背会这张表，滑动窗口直接通杀）



|     问题     | 最小覆盖子串（强制全覆盖） |  灵梦背单词（最大化种类）  |
| :----------: | :------------------------: | :------------------------: |
| **移 right** |     一直移，遍历完为止     |     一直移，遍历完为止     |
| **缩 left**  |    只有窗口**合法**才缩    |   每次**必缩**，能缩就缩   |
| **更新答案** |    仅**合法窗口**时更新    | sum 增加重置，收缩后取最小 |
| **核心口诀** |       先达标，再缩窗       |       先最多，再缩窗       |



PS：最大化种类是最小覆盖子串的超集，所以可以统一使用，是指稍微麻烦，我在题5都实现了，可以看一下
...
当我的窗口符合条件之后，保存或者更新当前的结果，强制移动i，然后更新判断窗口是否合法的变量，他不在乎是否会破坏合法窗口，因为已经结果已经保存或者更新了，然后在扩大窗口寻找新的合法窗口，然后继续更新。

最小覆盖问题使用最大化种类思路求解就是：

**重要的是窗口收缩条件是一样的**，这里就是已经假定了最大化种类是多少，然后就会**死命**维护这个最大化种类数目，结果就更新在当我们这个合法区间的左侧必不可少的时候判断是否更新，因为当左侧必不可少时候，左侧就无法再收缩了，此时较之前长度可能减少。
...
[A01 高精度算法 加法_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1UG4y1B7ur/?spm_id_from=333.337.top_right_bar_window_history.content.click&vd_source=86d06d10a03dcd75fe3a632194bc3c3c)
...
[P1601 高精度加法 - 洛谷](https://www.luogu.com.cn/problem/P1601#ide)



## 2.高精度减法

[P2142 高精度减法 - 洛谷](https://www.luogu.com.cn/problem/P2142#ide)

思路：

1. 将高精度数组利用字符串读入
2. 将高精度数字字符串**翻转转存**到整形数组当中
3. 从低位到高位，逐位相加，借位，存差
4. 将结果数组C依次从高位到低位输出

注意错点：

1. 首先如何判断两个高精度数字的大小，决定是否要先输出负号

   先判断位数，在逐位比较（最好看看源代码，这里比较使用!=)

2. 结果数组C里面，可能会有前导0，如何去除前导0
...
[P1303 A*B Problem - 洛谷](https://www.luogu.com.cn/problem/P1303#ide)

思路：

1. 高精度数->字符串
2. 字符串翻转存入整形数组
3. 从低位到高位，相乘，进位，取余，去除前导0
4. 输出结果

易错点：

理解乘法函数逻辑中，双重循环的意义，第一层i控制结果从上个结果的哪一位开始变化，j表示从i位置向前改变，并且因为是只有一个C[]用来累加上每一位乘数乘以被乘数，所以使用+=
...
1. 高精度数字使用字符串存储
2. 字符串存入执行数组，达到用数组模拟高精度数的目的
3. **从高位到低位**，更新当前被除数，存商，更新余数为再一次更新被除数做准备

易错点：

这里中间结果余数使用long long型
...
1. 加减乘都是从低位到高位，除法是从高位到低位运算，**理解倒转就是一个抽象理解成一个数组起点在右侧向左边延伸**
2. **一定要注意运算过程产生的前导0，不是后导0**，即是倒转数组左侧的0要消除，以免后续再次翻转得到答案时错误
